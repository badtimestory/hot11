...
public class FileManager {
	...
	public String save(MultipartFile multipartFile,String path) throws Exception{
		// 1.어디에 저장할거냐 
		//ex) path=/resources/upload/member
		...
		if(!file.exists()) {
			...
			file.mkdirs(); //중간에 폴더가 없으면 폴더를 생성해줌
		...
		String fName=l+"_"+oName;
		System.out.println("fileName:"+fName);
		
		// 2. UUID
		fName=UUID.randomUUID().toString();
		//static 클래스 메서드임 리턴이 string이 아니라서 toString으로 변환
		fName=fName+"_"+oName;
		System.out.println("UUID: "+fName);
